...
app.get('/api/v1/barcode/:id', async (req, res) => {
  ...
  if (barcodeId === '000000000000000') {
    return res.status(404).json({ message: 'Pet not found' });
  }
  if (barcodeId === '111111111111111') {
    mock1111.zoekresultaat.Identificatienummer = barcodeId;
    mock1111.zoekresultaat.Diernaam = 'Simba';
    mock1111.zoekresultaat.Diersoort = 'Kat';
    mock1111.zoekresultaat.Woonplaats_houder = 'Rotterdam';
  ...
  if (barcodeId === '222222222222222') {
    setTimeout(() => {
      mock1111.zoekresultaat.Identificatienummer = barcodeId;
      mock1111.zoekresultaat.Diernaam = 'Rocky';
      mock1111.zoekresultaat.Diersoort = 'Hond';
      mock1111.zoekresultaat.Woonplaats_houder = 'Amsterdam';
      return res.json(mock1111)
    }, 5000); // 5 intentional latency
    return;
  }
  if (barcodeId === '333333333333333') {
    return res.status(404).json({ message: 'Toegang geweigerd!' });
  }

  mock1111.zoekresultaat.Identificatienummer = barcodeId;
  mock1111.zoekresultaat.Diernaam = 'Bob';
  mock1111.zoekresultaat.Diersoort = 'Kat';
  mock1111.zoekresultaat.Woonplaats_houder = 'Den Haag';
